...
|  7 | Externe Daten         | Alle Objekte mit Positions/-, Rotations/- und Skalierungsdaten, welche im Editor schon vorab gebaut wurden, sind extern im JSON Format abgespeichert und werden zur Laufzeit von der FUDGE Engine verarbeitet und ausgeführt. Auch Objekte die später neu instantiert werden, werden über den selben Weg in das JSON File mit eingespeist und im nächsten Frame weiterverabeitet.                                                                            |
...
|  9 | Subklassen            | Es wird voraussichtlich mit vielen ComponentScript Methoden gearbeitet werden die schon eine Subklasse darstellen. Ebenfalls wird voraussichtlich von der Node Klasse abgeleitet werden.|
