        ...
        constructor(_x: number) {
            ...
            for (let i: number = 0; i < 5; i++) {
                for (let j: number = 0; j < 5; j++) {
                    ...
                    let randomMaterial: fCore.Material = new fCore.Material("Green", fCore.ShaderUniColor, new fCore.CoatColored(new fCore.Color(Math.random(), Math.random(), Math.random(), Math.random())));
