...
(function (basketBallBattleRoyale) {
    ...
    function createAvatar() {
        ...
        avatarNode.addComponent(new fCore.ComponentTransform(fCore.Matrix4x4.TRANSLATION(fCore.Vector3.Y(2))));
    ...
    function isGrabbingObjects() {
        if (basketBalls != undefined) {
            ...
            if (fCore.Keyboard.isPressedOne([fCore.KEYBOARD_CODE.R]) && isGrabbed == true) {
                ...
                playerForward = fCore.Vector3.Z();
                playerForward.transform(avatarNode.mtxWorld, false);
                let distance = fCore.Vector3.DIFFERENCE(basketBalls[0].mtxWorld.translation, players[0].getChild(0).getChild(1).getComponent(fCore.ComponentMesh).mtxWorld.translation);
                console.log("aktuellePower: " + distance.magnitude * 25, "nur magnitude" + distance.magnitude);
                let trigger = players[0].getChild(0).getChild(1);
                trigger.getComponent(fCore.ComponentRigidbody).addEventListener("TriggerEnteredCollision" /* TRIGGER_ENTER */, hndTrigger);
                let distanceMag = distance.magnitude;
                if (distanceMag > 6)
                    basketBalls[0].getComponent(fCore.ComponentRigidbody).applyImpulseAtPoint(new fCore.Vector3(playerForward.x * kickStrength, distanceMag * 25, playerForward.z * kickStrength), avatarNode.mtxWorld.translation);
                else
                    basketBalls[0].getComponent(fCore.ComponentRigidbody).applyImpulseAtPoint(new fCore.Vector3(playerForward.x * kickStrength, 22 / distanceMag * 25, playerForward.z * kickStrength), avatarNode.mtxWorld.translation);
            }
        }
    }
